...
alter table `follow` add constraint fk_member_id foreign key(`from`) references member(member_id) on delete cascade;
alter table `follow` add constraint fk_member_id foreign key(`to`) references member(member_id) on delete cascade;
