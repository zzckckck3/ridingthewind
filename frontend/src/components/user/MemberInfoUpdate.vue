<template>
    <v-container class="update-container">
        ...
        <v-form class="update-form" @submit="submitForm">
            <v-row>
                <v-col cols="12" md="12">
                    <v-text-field label="아이디" v-model="id" required readonly></v-text-field>
                </v-col>
<!--                <v-col cols="12" md="12">-->
...
<!--                </v-col>-->
            ...
            <v-row>
                <v-col cols="12" md="6">
                    <v-text-field label="생일" v-model="birth" type="date" required></v-text-field>
                </v-col>
                <v-col cols="12" md="12">
                    <v-text-field label="닉네임" v-model="nickname" required></v-text-field>
                </v-col>
            </v-row>
            <v-btn class="signup-btn" @click="update" color="primary">수정완료</v-btn>
        </v-form>
    </v-container>
</template>
...
export default {
    ...
    methods: {
        ...
        update(){
            if(this.id === '' || this.name === '' || this.phoneNumber === ''||
                this.emailId === '' || this.emailDomain === '' || this.birth === '' || this.nickname === ''){
                alert("값을 모두 입력해주세요");
                return;
            }
